...
QChartImage::QChartImage(QWidget *parent) : QLabel(parent)
{
    // High dpi support
    m_dpi = 0;
    m_toImageScaleX = 1;
...
static class QASE_CursorManager
{
  ...
  public:
    QASE_CursorManager() : hZoomInCursor(0), hZoomOutCursor(0), hNoZoomCursor(0)
    {
    }
    ~QASE_CursorManager()
    {
...
QChartViewer::QChartViewer(QWidget *parent) : QChartImage(parent)
{
...
static class QtResourceLoader
{
  public:
    ...
    {
        QFile f(QString(":/") + id);
        return f.open(QIODevice::ReadOnly) && (0 < (*len = f.size())) && (0 != (*data = allocator(*len))) &&
               f.read(*data, *len);
    }
...
void QChartViewer::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if ((m_delayedMouseEvent && (timeBetweenMouseMove < 250)) || (timeBetweenMouseMove < 10))
    {
        ...
        m_delayedMouseEvent = new QMouseEvent(event->type(), event->pos(), event->globalPos(), event->button(),
...
void QChartViewer::wheelEvent(QWheelEvent *event)
{
    ...
    bool hasMouseWheelZoom =
        isMouseOnPlotArea() && onMouseWheelZoom(getPlotAreaMouseX(), getPlotAreaMouseY(), event->angleDelta().y());
        ...
        case Chart::MouseUsageZoomIn:
            if (canZoomIn(m_zoomDirection))
            {
                if (isDrag(m_zoomDirection, event))
                {
                    ...
                    hasUpdate = zoomTo(m_zoomDirection, toImageX(minX), toImageY(minY), toImageX(minX + spanX),
                                       toImageY(minY + spanY));
                }
                else
...
void QChartViewer::onPlotAreaMouseDrag(QMouseEvent *event)
{
    ...
    case Chart::MouseUsageScroll: {
        ...
        if (m_isDragScrolling || isDrag(m_scrollDirection, event))
        {
            ...
            if (dragTo(m_scrollDirection, toImageX(event->x() - m_plotAreaMouseDownXPos),
                       toImageY(event->y() - m_plotAreaMouseDownYPos)))
...
QViewPortControl::QViewPortControl(QWidget *parent) : QChartImage(parent)
{
    m_ChartViewer = 0;
    m_Chart = 0;
