    ...
    QDialog(parent)
{
    ...
    m_AlphaCB = new QCheckBox("Alpha Series", frame);
    m_AlphaCB->setChecked(true);
    m_AlphaCB->setGeometry(4, 130, 112, 20);
    connect(m_AlphaCB, SIGNAL(clicked()), SLOT(onSelectDataSeries()));

    m_BetaCB = new QCheckBox("Beta Series", frame);
    m_BetaCB->setChecked(true);
    m_BetaCB->setGeometry(4, 150, 112, 20);
    connect(m_BetaCB, SIGNAL(clicked()), SLOT(onSelectDataSeries()));

    m_GammaCB = new QCheckBox("Gamma Series", frame);
    m_GammaCB->setChecked(true);
    m_GammaCB->setGeometry(4, 170, 112, 20.0);
    connect(m_GammaCB, SIGNAL(clicked()), SLOT(onSelectDataSeries()));
...
void ZoomScrollTrack2::drawChart(QChartViewer *viewer)
{
    ...
    if (m_AlphaCB->isChecked())
        layer->addDataSet(viewPortDataSeriesA, 0xff3333, "Alpha");
    if (m_BetaCB->isChecked())
        layer->addDataSet(viewPortDataSeriesB, 0x008800, "Beta");
    if (m_GammaCB->isChecked())
...
//
void ZoomScrollTrack2::onMouseUsageChanged(QAbstractButton *b)
{
...
//
void ZoomScrollTrack2::onSelectDataSeries()
{
    m_ChartViewer->updateViewPort(true, true);
}
